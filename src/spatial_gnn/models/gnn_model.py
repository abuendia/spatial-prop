...
class GNN(torch.nn.Module):
    '''
    GNN model for spatial transcriptomics data

    Arguments:
        hidden_channels [int] - number of hidden channels
        input_dim [int] - input dimension
        output_dim [int] - output dimension
        inject_dim [int] - dimension of inject feature
        num_layers [int] - number of layers (k-hop)
        method [str] - method to use for GNN
        pool [str] - pool method to use ("mean", "add", "max", "center", "SAGPooling", "ASAPooling", "GlobalAttention")
        genept_embeddings [dict] - dictionary of genept embeddings
        genept_strategy [str] - strategy to use for genept
        celltypes_to_index [dict] - dictionary of cell types to index
        predict_celltype [bool] - whether to predict cell type
        train_multitask [bool] - whether to train multitask network with celltype
        celltype_model [torch.nn.Module] - separately trained cell type model
        ablate_gene_expression [bool] - whether to ablate gene expression
        use_one_hot_ct [bool] - whether to use one-hot cell type
        predict_residuals [bool] - whether to predict residuals
    '''
    def __init__(
        self,
        hidden_channels,
        ...
        # Ablation mode: MLP that only uses cell type (no gene expression)
        ...
        # Zero out weights and biases of last layers if predicting residuals
